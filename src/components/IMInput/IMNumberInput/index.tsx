...
import { CurrencyCodes } from '../../../constants/strings';
import { indianCurrencyCommaSeparator } from '../../../utils/common';
import { InputVariant, NumberInputType } from '../helper';
import IMBaseInput from '../IMBaseInput';
  ...
  const currencyAdornment = () => {
    if (props.type === 'currency') {
      if (props.countryCode === 'IN') {
        return (
          <InputAdornment
            testId={`${props.testId}-adornment`}
            text={{ name: CurrencyCodes.IN, textStyle: [styles.currencyText, props.style?.startAdornmentText] }}
          />
        );
      }
    }
